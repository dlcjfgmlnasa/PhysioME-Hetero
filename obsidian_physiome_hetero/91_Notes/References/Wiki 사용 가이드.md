...
```
obsidian_physiome_hetero/           ← Obsidian vault root 후보
...
1. **Vault 루트**: `C:\Projects\PhysioME\obsidian_physiome_hetero` (또는 `C:\Projects\PhysioME` 전체)
   - `obsidian_physiome_hetero/`만 vault 루트로 잡으면 코드와 분리되어 깔끔
...
- 코드 파일 링크: `[[../../models/physiome/model.py]]` (vault root가 obsidian_physiome_hetero일 때)
